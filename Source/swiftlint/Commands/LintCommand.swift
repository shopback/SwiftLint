...
struct LintOptions: OptionsProtocol {
    ...
    static func create(_ path: String) -> (_ useSTDIN: Bool) -> (_ configurationFile: String) -> (_ strict: Bool) -> (_ lenient: Bool) -> (_ forceExclude: Bool) -> (_ useScriptInputFiles: Bool) -> (_ benchmark: Bool) -> (_ reporter: String) -> (_ quiet: Bool) -> (_ cachePath: String) -> (_ ignoreCache: Bool) -> (_ enableAllRules: Bool) -> (_ useBaseline: Bool) -> (_ paths: [String]) -> LintOptions {
        return { useSTDIN in { configurationFile in { strict in { lenient in { forceExclude in { useScriptInputFiles in { benchmark in { reporter in { quiet in { cachePath in { ignoreCache in { enableAllRules in { useBaseline in { paths in
            let allPaths: [String]
            if !path.isEmpty {
                allPaths = [path]
            } else {
    ...
    static func evaluate(_ mode: CommandMode) -> Result<LintOptions, CommandantError<CommandantError<()>>> {
        return create
            ...
            <*> mode <| Option(key: "use-baseline", defaultValue: false,
                               usage: "save a list of violations if there is no baseline file, and then" +
                                      "use the list as a baseline to ignore old issues and only report new ones")
